...
class LatexPaletteDock(QDockWidget):
    """Collapsible dock containing mathematical LaTeX snippets with focus-free buttons."""
    ...
    def __init__(self, parent=None):
        ...
        tabs.addTab(self._build_calculus_tab(), "Kalkül")
        tabs.addTab(self._build_greek_tab(), "Griechisch")
        tabs.addTab(self._build_symbols_tab(), "Symbole")
        tabs.addTab(self._build_matrices_tab(), "Matrizen")
    ...
    def _build_calculus_tab(self) -> QWidget:
        container = QWidget()
    ...
    def _build_greek_tab(self) -> QWidget:
        container = QWidget()
        grid = QGridLayout(container)
        grid.setSpacing(4)
        letters = [
    ...
    def _build_symbols_tab(self) -> QWidget:
        ...
        syms = [
            ...
            ("≈", r"\approx"), ("∈", r"\in"), ("∀", r"\forall"), ("∃", r"\exists"),
            ("→", r"\to"), ("⇒", r"\implies"), ("⇔", r"\iff"), ("∇", r"\nabla"),
        ]
        for idx, (lbl, code) in enumerate(syms):
            grid.addWidget(self._create_btn(lbl, code, code), idx // 4, idx % 4)
        return self._wrap(container)

    def _build_matrices_tab(self) -> QWidget:
        container = QWidget()
        grid = QGridLayout(container)
        grid.setSpacing(4)
        mat_2x2 = "\\begin{pmatrix} a & b \\\\ c & d \\end{pmatrix}"
        mat_3x3 = "\\begin{pmatrix} a & b & c \\\\ d & e & f \\\\ g & h & i \\end{pmatrix}"
        vec_col = "\\begin{pmatrix} x \\\\ y \\end{pmatrix}"
        vec_row = "\\begin{pmatrix} x & y \\end{pmatrix}"
        ...
        grid.addWidget(self._create_btn("Vektor (Spalte)", vec_col, "Spaltenvektor"), 1, 0)
        grid.addWidget(self._create_btn("Vektor (Zeile)", vec_row, "Zeilenvektor"), 1, 1)
        return self._wrap(container)
